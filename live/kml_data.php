...
require_once('datasources.inc.php');
...
$source = $datasources[$_GET['source']];
// Get the data from the Database Table (planet_osm_point)
$sql = "SELECT ".$source['id']." AS id, ST_X(geom) as lon, ST_Y(geom) as lat FROM ".$source['table']." WHERE (box(point(" . $bbox_south . "," . $bbox_west . "),point(" . $bbox_east . "," . $bbox_north . ")) ~ (geom)) LIMIT 1000";
                //or with transform:
